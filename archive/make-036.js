   ...
   const _=repeat(" ",indent)
   {
    const indent=_
    {
     const _=scan(x,is_lexem)
     {
      const a=_
      {
       const _=replace(a,"[","arr")
       {
        const a=_
        {
         const _=rewrite_obj(a)
         {
          const a=_
          {
           const _=replace(a,"!","not")
           {
            const a=_
            {
             const _=filter(a)
 ...
 function set_nearest_line(x,y)
 {
  ...
  const _=null
  {
   let r=_
   {
    for(const v of (typeof(x)==="function")?x():x)
    {
     const _=(typeof(v.n)==="function")?v.n():v.n
     {
      const n=_
      {
       if(gte(n,y))
