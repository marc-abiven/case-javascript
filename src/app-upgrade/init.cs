fn init x:etc
 ...
 ssh_system token "ssh" login_vps "rm" "--force" "--recursive" tmp
 ssh_system token "ssh" login_vps "rm" "--force" remote_bin
 ssh_system token "ssh" login_vps "mkdir" tmp
 ssh_system token "rsync" "--compress" "--perms" bin target_tmp
 ssh_system token "ssh" login_vps "sudo" "rm" "--force" bin
 ssh_system token "ssh" login_vps "sudo" "cp" "--force" remote_bin bin
 ssh_system token "ssh" login_vps "rm" "--force" remote_bin
 
 let version_remote ssh_execute token "ssh" login_vps "node" "--version"
