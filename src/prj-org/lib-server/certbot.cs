fn certbot domains:etc
 ...
 fn load_config
  let file path_concat config_data "certbot.cson"
  let data obj

  if is_file file
   let o cson_load file
 ...
 fn get_expiry_dates
  let r obj
  let child sudo os_run "certbot" "certificates"
  let lines child.out
 ...
 forin data
  let now time_get
  let now trunc now
  let refresh add v day //everyday

  if gt refresh now
   cont

  sudo os_run "certbot" "certonly" "--standalone" "--email" author "--agree-tos" "--keep-until-expiring" "--domain" k

  set data k now
 end

 let s cson_encode data
