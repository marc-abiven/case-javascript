fn init name:str user_mail:str x:etc
 ...
 forin users
  if v.human
   assign humans inc humans
 end

 //comment

 let comment inc humans
 let comment concat "#" comment
 let comment space free_server_sh "user" comment

 //create user

 sudo "useradd" "--create-home" name "--shell" shell "--comment" comment
 ...
 volatile "create" s //do not record the password in the log file
 ...
 let logo "src/prj-org/lib-org/res/freeserver-sh-2025-09-18-logo-003.png"
 ...
 let span1 h_span s1
 let wbr h_wbr //avoid highlight in gmail
 let span2 h_span ".sh"

 let b h_b

 h_push b span1
 h_push b wbr //avoid gmail to transform the mail to an url
 h_push b span2
