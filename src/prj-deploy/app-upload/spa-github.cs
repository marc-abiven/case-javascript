gn spa_github upload:bool name:str user callback
 ...
 let pattern concat "*/" spa "/res"
 let spa_res fs_find "src" pattern
 ...
 sudo_fs_copy tmp www
 sudo_fs_change_mode www "777"

 //git

 if upload
  run redact token git_upload tmp user url //hide app token in the output
end
